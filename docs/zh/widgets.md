...
`JellyBobbin\Admin\Widgets\Box`用来生成box组件：
...
`JellyBobbin\Admin\Widgets\Collapse`类用来生成折叠插件：
```php
...
`JellyBobbin\Admin\Widgets\Form`类用来快速构建表单：
...
`JellyBobbin\Admin\Widgets\InfoBox`类用来生成信息展示块：
...
`JellyBobbin\Admin\Widgets\Tab`类用来生成选项卡组件：
...
`JellyBobbin\Admin\Widgets\Table`类用来生成表格：
